...
export class MTLLoader {
  public static load(gl: WebGLRenderingContext, baseurl: string) {
    ...
    return fetchRes(baseurl).then((content: string) => {
        ...
        .filter((line) => !!line)
        .forEach((line) => {
            currentMaterial = MTLLoader.handleSingleLine(
            gl,
            home,
            line,
            materials,
  ...
  protected static mapPattern = /(map_[^\s]+|bump|disp|decal)\s.+/gm;
  ...
  private static handleSingleLine(
    ...
    switch (firstVar) {
        ...
        if (currentMaterial instanceof BlinnPhongMaterial) {
            ...
            MTLLoader.getVector(MTLLoader.specularePattern, line),
            );
        }
        break;
        case "Ns":
        ...
        break;
        case "map_Ka":
        currentMaterial.setMainTexture(
            new Texture2D(gl, home + MTLLoader.getImageUrl(line)),
        );
        break;
        case "map_Kd":
        currentMaterial.setMainTexture(
        ...
        break;
        case "map_d":
        currentMaterial.setAlphaMap(
            new Texture2D(gl, home + MTLLoader.getImageUrl(line)),
        );
        break;
        case "map_bump":
        currentMaterial.setBumpMap(
            new Texture2D(gl, home + MTLLoader.getImageUrl(line)),
        );
        break;
        case "bump":
        ...
        break;
        case "disp":
        currentMaterial.setDisplamentMap(
            new Texture2D(gl, home + MTLLoader.getImageUrl(line)),
        );
        break;
        case "decal":
        currentMaterial.setStencilMap(
            new Texture2D(gl, home + MTLLoader.getImageUrl(line)),
        );
        break;
        case "Pr":
        ...
        currentMaterial.setRoughness(
            MTLLoader.getNumber(MTLLoader.roughnessPattern, line),
        );
        break;
        case "Pm":
        if (currentMaterial instanceof BlinnPhongMaterial) {
        ...
        currentMaterial.setMetallic(
            MTLLoader.getNumber(MTLLoader.metallicPattern, line),
        );
        break;
        default:
        break;
